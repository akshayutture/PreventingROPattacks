...
class Core(cmd.Cmd):
	...
	'''
	VERSION WITH FAR CALLS
	#Checks for a Call near, absolute indirect, address given in r/m16 or r/m32 or Call far, absolute, address given in operand m16:16 or m16:32 (ff \3 or ff \2 opcode)
    def __checkIfPrevInstrCallffOpcode(self,prev):
        if ord(prev[7])==255 and ((ord(prev[8]))/8)%8==2:
            return True
        if ord(prev[5])==255 and ((ord(prev[6]))/8)%8==2:
            return True
        if ord(prev[3])==255 and ( ((ord(prev[4]))/8)%8==2 or ((ord(prev[4]))/8)%8==3):
            return True
        if ord(prev[1])==255 and ((ord(prev[2]))/8)%8==3:
            return True
        return False
    	
	#Set of additional functions added to check if the previous instruction was a call
    def __checkIfPrevInstrCallInstr(self,prev):
        #Check all the variations of the call instruction in the previous instruction
        if ord(prev[4])==232  or ord(prev[6])==232: #Call near, relative, displacement relative to next instruction (e8 opcode)
        	return True
        if ord(prev[4])==154  or ord(prev[2])==154: #	Call far, absolute, address given in operand (9a opcode)
        	return True
        if self.__checkIfPrevInstrCallffOpcode(prev): #Call near, absolute indirect, address given in r/m16 or r/m32 or Call far, absolute, address given in operand m16:16 or m16:32 (ff \3 opcode)
        	return True
        return False
	'''
	
	'''
	VERSION WITHOUT FAR CALLS (since the flat memory model of OSs does not use segmentation anyway). So CS will never be changed
	'''	
	#Checks for a Call near, absolute indirect, address given in r/m16 or r/m32 (ff opcode)
    def __checkIfPrevInstrCallffOpcode(self,prev):
        if ord(prev[7])==255 and ((ord(prev[8]))/8)%8==2:
            return True
        if ord(prev[5])==255 and ((ord(prev[6]))/8)%8==2:
            return True
        if ord(prev[3])==255 and ((ord(prev[4]))/8)%8==2:
            return True
        return False
    ...
    def __checkIfPrevInstrCallInstr(self,prev):
        ...
        if ord(prev[4])==232  or ord(prev[6])==232: #Call near, relative, displacement relative to next instruction (e8 opcode)
        	return True
        if self.__checkIfPrevInstrCallffOpcode(prev): #Call near, absolute indirect, address given in r/m16 or r/m32  (ff opcode)
        	return True
        return False
    ...
    def __lookingForGadgets(self):
        ...
        for gadget in self.__gadgets:
            ...
            if self.__checkIfPrevInstrCallInstr(prev)==True:
            	numberOfValidGadgets = numberOfValidGadgets + 1
            	print "Valid Gadget"
            else:
            	print "Invalid Gadget"

        print("\nUnique gadgets found: %d" %(len(self.__gadgets)))
        print("\nValid gadgets found: %d" %(numberOfValidGadgets))
        return True
